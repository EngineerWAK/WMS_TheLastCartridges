...
if (!(_targetUID in _playerAlreadyConnected)&&{count _targetUID == 17}) then {
...
if (WMS_MissionDebug) then {diag_log format ["[PLAYERJOINING]|WAK|TNA|WMS| player: %1, UID: %2, Kills: %3, Money: %4, Score: %5, Death: %6", _playerUnit, _targetUID, _playerKills,_playerMoney,_playerScore,_playerDeath]};
