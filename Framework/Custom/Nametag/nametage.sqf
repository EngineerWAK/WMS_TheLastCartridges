...
if !(isServer) then {
	...
	while{true} do {
		...
		sleep 0.5;
		if((isPlayer cursorTarget) && {(alive cursorTarget)} && {(side cursorTarget == side player)} && {!((getplayerUID cursorTarget) in _IP_ActiveList)} && {((player distance cursorTarget) < 1200)}) then {
