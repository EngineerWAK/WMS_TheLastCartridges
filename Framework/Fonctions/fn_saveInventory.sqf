/*
	...
	ARRAY - saved value
*/
//modified by {|||TNA|||}WAKeupneo to export the exact amount of ammo/mags not just fully reloaded mags, this is exported as "_custom"
//Not Used, i moved the essential in WMS_fnc_saveRespawnData
...
//--- Get current values
private ["_export","_magazinesAmmo","_uniformItems","_vestItems","_backpackItems"];
_magazinesAmmo = magazinesAmmo player;
_uniformItems = [];
_vestItems = [];
_backpackItems = [];
{if !(_x isKindOf "CA_Magazine") then {_uniformItems pushBack _x}}forEach (uniformitems _center);
{if !(_x isKindOf "CA_Magazine") then {_vestItems pushBack _x}}forEach (vestitems _center);
{if !(_x isKindOf "CA_Magazine") then {_backpackItems pushBack _x}}forEach (backpackitems _center);

_export = [
	/* 00 */	[uniform _center,_uniformItems],
	/* 01 */	[vest _center,_vestItems],
	/* 02 */	[backpack _center,_backpackItems],
	/* 03 */	headgear _center,
	/* 04 */	goggles _center,
