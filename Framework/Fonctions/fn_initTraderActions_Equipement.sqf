...
if (false) then {diag_log format ["[INIT_TRADERS]|WAK|TNA|WMS|UPDATE: _itemsSelected %1", _itemsSelected]};
{
	...
	//_display = format ['<t color=%3> %1, %2 po$abs</t>',_itemName,_price, _color]; //NOPE, not with colors
	...
	[ //params ["_target", "_caller", "_actionId", "_arguments"];
		...
		[
			...
			",
			[_item,_price,_respectLevel,_traderType], //argument accessible in the scri$ (_this select 3)
			1,
			true,
			true,
			"",
